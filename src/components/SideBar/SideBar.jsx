    ...
    return (<S.SidebarContainer>
      <S.NavList>
        {navItems.map((item) => {
            let isActive = false;
            if (item.path === '/btc')
                isActive = isBtcActive;
            else if (item.path === '/oral-systemic')
                isActive = isOralSystemicActive;
            else if (item.path === '/partners')
                isActive = isPartnersActive;
            else if (item.path === '/toothbrush')
                isActive = isToothbrushActive;
            else if (item.path === '/operatory')
                isActive = isOperatoryActive;
            else if (item.path === '/accessories')
                isActive = isAccessoriesActive;
            else
                isActive = location.pathname === item.path;
            return (<S.NavItem key={item.path}>
              <S.NavButton to={item.path} className={isActive ? 'active' : ''}>
                <S.NavIcon src={isActive ? item.activeIcon : item.inactiveIcon} alt={item.name}/>
