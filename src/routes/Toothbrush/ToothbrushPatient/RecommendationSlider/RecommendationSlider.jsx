...
const createSlides = (selection) => {
    if (!selection || !SLIDE_CONFIGS[selection])
        return [];
    return SLIDE_CONFIGS[selection].map((slide) => (<S.Slide key={slide.key}>
      <S.ProductImage src={slide.src} alt={slide.alt}/>
    </S.Slide>));
};
export const RecommendationSlider = ({ selection }) => {
    const slides = createSlides(selection);
    return (<S.CarouselWrapper>
            <Flickity key={selection || 'default'} className={'carousel'} elementType={'div'} options={flickityOptions} disableImagesLoaded={false} static={false}>
            {slides}
            </Flickity>
        </S.CarouselWrapper>);
};
