...
export default function BtcPatient() {
    ...
    const selectedEmbrasure = useMemo(() => {
        if (activeImages.embrasure1)
            return "embrasure1";
        if (activeImages.embrasure2)
            return "embrasure2";
        if (activeImages.embrasure3)
            return "embrasure3";
    }, [activeImages]);
    const selectedMotivation = useMemo(() => {
        if (activeImages.motivated)
            return "motivated";
        if (activeImages.unmotivated)
            return "unmotivated";
    }, [activeImages]);
    const selectedCondition = useMemo(() => {
        if (activeImages.periodontal)
            return "periodontal";
        if (activeImages.appliance)
            return "appliance";
        if (activeImages.orthodontic)
            return "orthodontic";
    }, [activeImages]);
    const currentRecommendations = useMemo(() => {
        if (!selectedEmbrasure || !selectedMotivation)
            return null;
    ...
    const toggleImage = (imageKey, rowIndex) => {
        if (rowIndex > 0) {
            const previousRowKey = `row${rowIndex}`;
            const hasPreviousRowSelection = rowGroups[previousRowKey].some(key => activeImages[key]);
            if (!hasPreviousRowSelection)
                return;
        }
        setActiveImages(prev => {
            ...
            const shouldClearSubsequentRows = !newState[imageKey] ||
    ...
    const hasRowSelection = (rowIndex) => {
    ...
    return (<S.PatientContainerOuter>
    <S.PatientContainer>
      <S.ChoicesContainerOuter>
        <S.ChoicesContainerInner>
          ...
          {/* Row 1 */}
          <S.Row>
            {rowGroups.row1.map(key => (<S.ImgWrapper key={key} onClick={() => toggleImage(key, 0)}>
                <img src={activeImages[key] ? imageSources[key].active : imageSources[key].normal} alt={`${key.replace(/([A-Z])/g, ' $1').trim()} option`}/>
              </S.ImgWrapper>))}
          </S.Row>
          ...
          <S.Row2>
            ...
            <S.ImgWrapper3 onPointerDown={() => toggleImage("unmotivated", 1)}>
          ...
          {/* Row 3 */}
          <S.Row>
            {rowGroups.row3.map(key => (<S.ImgWrapper key={key} onClick={() => toggleImage(key, 2)}>
                <img src={activeImages[key] ? imageSources[key].active : imageSources[key].normal} alt={`${key} case`}/>
              </S.ImgWrapper>))}
          </S.Row>
        </S.ChoicesContainerInner>
